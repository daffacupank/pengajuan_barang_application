...
import 'update_status_kaleb_page.dart';

class PenggunaHomePage extends StatefulWidget {
  const PenggunaHomePage({super.key});

  @override
  State<PenggunaHomePage> createState() => _PenggunaHomePageState();
}

class _PenggunaHomePageState extends State<PenggunaHomePage> {
  bool isKaleb = false;
  ...
  Future<void> _loadKalebStatus() async {
    ...
    try {
      ...
      final kaleb = (user['kaleb'] ?? user['is_kaleb'] ?? '0').toString();
      if (mounted) {
        setState(() => isKaleb = kaleb == '1' || kaleb.toLowerCase() == 'true');
      }
    } catch (_) {
      // Kalau format user tidak valid, menu Kaleb tidak ditampilkan.
    }
  }
                ...
                children: [
                  ...
                  GridView.count(
                    ...
                    children: [
                      ...
                      if (isKaleb)
                        _MenuCard(icon: Icons.manage_search_rounded, title: 'Update Status', subtitle: 'Lihat update laporan user', onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const UpdateStatusKalebPage()))),
