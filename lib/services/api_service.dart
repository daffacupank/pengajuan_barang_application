...
class ApiService {
  ...
  static Future<Map<String, dynamic>> updateLaporanKaleb({
    required int idLaporan,
    required String status,
    required String keteranganAdmin,
  }) async {
    ...
    try {
      ...
      final response = await http
          .put(
        Uri.parse(
          '$baseUrl/api/pengguna/kaleb/update-laporan',
        ),
      ...
      return response.statusCode == 200
        ...
        data['message'] ??
            ...
            'Gagal update (${response.statusCode})'
  ...
  static Future<Map<String, dynamic>> getUpdateStatusKaleb() async {
    try {
      final token = await getToken();
      if (token == null) return {'success': false, 'message': 'Sesi login tidak ditemukan'};

      // Endpoint utama. Kalau backend sampeyan pakai nama route berbeda,
      // ubah cukup bagian URL ini saja.
      final endpoints = [
        '$baseUrl/api/pengguna/kaleb/update-status',
        '$baseUrl/api/pengguna/update-status-kaleb',
        '$baseUrl/api/pengguna/update-laporan-kaleb',
        '$baseUrl/api/kaleb/update-status',
      ];

      Map<String, dynamic> lastError = {'success': false, 'message': 'Endpoint update status Kaleb belum tersedia di backend.'};
      ...
      for (final url in endpoints) {
        ...
        if (response.statusCode == 401 || response.statusCode == 403) {
          await logout();
          return {'success': false, 'message': data['message'] ?? 'Sesi berakhir'};
        }

        // 404 berarti coba endpoint fallback berikutnya.
        if (response.statusCode != 404) {
          lastError = {'success': false, 'message': data['message'] ?? 'Error ${response.statusCode}'};
        }
      }
